...
void main() async {
  ...
  testWidgets('test', (tester) async {
    await sect('애플리케이션 실행', () async {
      app.main();

      await Future.delayed(Duration(seconds: 1));
    }, tester);
    await sect('잘못된 이메일 형식으로 로그인 시도', () async {
    ...
    await sect('입력 필드 초기화 후 짧은 비밀번호로 로그인 시도', () async {
      final f1 = find.byType(TextField).at(0);
      final f2 = find.byType(TextField).at(1);
      final f3 = find.byType(ElevatedButton).last;
    ...
    await sect('입력 필드 초기화 후 정상 값으로 로그인 시도', () async {
      final f1 = find.byType(TextField).at(0);
      final f2 = find.byType(TextField).at(1);
      final f3 = find.byType(ElevatedButton).last;
    ...
    await sect('하단 네비게이션의 "탐색" 탭 클릭', () async {
    ...
    await sect('음반 상태 필터 순차 클릭', () async {
    ...
    await sect('아래로 스크롤하여 15번째 상품 아이템까지 이동', () async {
    ...
    await sect('15번째 상품 아이템 클릭', () async {
      final f1 = find.byType(AlbumCard, skipOffstage: false);

      await tester.ensureVisible(f1.at(14));
      await tester.pumpAndSettle();
      await tester.tap(f1.at(14));
    }, tester);

    await sect('앱바 우측 관심 아이콘 버튼 클릭', () async {
      final f1 = find.byType(LikeButton).first;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('뒤로가기 버튼 클릭 후 하단 네비게이션 “관심상품“ 탭 클릭', () async {
    ...
    await sect('등록된 관심 상품 아이템 리스트 표시 확인', () async {}, tester);
    await sect('첫 번째 아이템의 관심(하트) 아이콘 버튼 클릭', () async {
      final f1 = find.byType(LikeButton).first;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('비활성화 관심 상품 아이템 리스트 표시 확인', () async {}, tester);

    await sect('하단 네비게이션 "홈" 탭 클릭', () async {
      final f1 = find.text('홈').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);
    await sect('하단 네비게이션 "관심상품" 탭 클릭', () async {
      final f1 = find.text('관심상품').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('비활성화 관심 상품 아이템 리스트 제거 확인', () async {}, tester);
  });
}
