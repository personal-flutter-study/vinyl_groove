...
import 'package:vinyl_groove_poc_8/main.dart' as app;
...
class Mock extends ImagePickerPlatform with MockPlatformInterfaceMixin {
  ...
  Future<XFile> getImage({
    ...
    return XFile.fromData(bytes, name: 'vinyl_sample.png');
  }
}
...
void main() async {
  ...
  testWidgets('test', (tester) async {
    ...
    await sec('첫 번째 상품 아이템 삭제 아이콘 버튼 클릭', () async {
      final f1 = find.byType(IconButton).at(1);
      await vis(f1, tester);
      await tester.tap(f1);
    }, tester);
    ...
    await sec('뒤로가기 버튼 클릭', () async {
      final f1 = find.byIcon(Icons.arrow_back).last;
      await vis(f1, tester);
      await tester.tap(f1);
    }, tester);
