...
class Mock extends ImagePickerPlatform with MockPlatformInterfaceMixin {
  ...
  Future<XFile> getImage({
    ...
    final img = await rootBundle.load('assets/vinyl_sample.png');
...
void main() async {
  ...
  testWidgets('test', (tester) async {
    ...
    await sect('하단 네비게이션 상품 등록 진입 버튼 클릭', () async {
    ...
    await sect('이미지 영역 클릭 후 “갤러리에서 선택” 메뉴 선택하여 이미지 선택', () async {
      final f1 = find
    ...
    await sect('가격 입력 상자 선택 후 새 가격 입력', () async {
      final f1 = find.byType(TextField).at(2);
      await tester.ensureVisible(f1);
    ...
    await sect('하단 네비게이션 "마이페이지" 탭 클릭', () async {
      final f1 = find.text('마이페이지').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);
    await sect('"내 등록 상품" 메뉴 클릭', () async {
      final f1 = find.text('내 등록 상품').first;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('등록된 첫 번째 상품 아이템 리스트에 표시 확인', () async {}, tester);

    await sect('첫 번째 상품 아이템 삭제 아이콘 버튼 클릭', () async {
    ...
    await sect('다이얼로그의 "삭제" 버튼 클릭', () async {
      final f1 = find.text('삭제').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('등록 상품 목록 빈 상태 확인', () async {
      final f1 = find.byIcon(Icons.arrow_back).first;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('"로그아웃" 버튼 클릭', () async {
      final f1 = find.text('로그아웃').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);

    await sect('로그아웃 확인 다이얼로그의 "로그아웃" 버튼 클릭', () async {
      final f1 = find.text('로그아웃').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);
    await sect('로그인 화면 정상 표시 확인', () async {
      final f1 = find.text('로그아웃').last;
      await tester.ensureVisible(f1);
      await tester.tap(f1);
    }, tester);
